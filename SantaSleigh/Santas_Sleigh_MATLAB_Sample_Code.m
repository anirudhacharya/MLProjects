...
presentsO = csvread('presents.csv', firstRow, firstCol);
presents = presentsO(1:195,:);
presentsO = presents;
...
% plot3(presentWidth(0:1000000), presentLength(0:1000000), presentHeight(0:1000000), '.')
% xlabel('width');
% ylabel('length');
% zlabel('height');
% title('Present Sizes');
...
for i = 1:numPresents
    ...
    if xs + presentWidth(i) > width + 1 % exceeded allowable width
        ...
        rowStats = [rowStats; numInRow xs presentWidth(i) ];
        xs = 1;
        numInRow = 0;
    end
    ...
    if ys + presentLength(i) > length + 1 % exceeded allowable length
        ...
        rowStats = [rowStats; numInRow xs presentWidth(i)];
        layStats = [layStats; numInLayer ys presentLength(i) -zs];
        xs = 1;
        ys = 1;
        numInLayer = 0;
    end
...
%Santas_Sleigh_MATLAB_Metric_Calculation_v2('presents.csv', 'submissionfile.csv')
